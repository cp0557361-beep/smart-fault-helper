...
export function SequenceChips({
  ...
  return <div className={cn('space-y-2', className)}>
      <div className="flex flex-wrap gap-1.5">
        {value.map(seq => <Badge key={seq} variant="secondary" className="gap-1 pr-1">
            {seq}
            {!disabled && <button type="button" onClick={() => removeSequence(seq)} className="ml-1 rounded-full hover:bg-muted p-0.5">
                <X className="w-3 h-3" />
              </button>}
          </Badge>)}
      </div>
